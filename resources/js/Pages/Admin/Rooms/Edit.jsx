...
export default function Edit({ auth, room, asramas }) {
    const { data, setData, put, processing, errors } = useForm({
        asrama_id: room.asrama_id || '',
    ...
    if (!asramas) {
        return (
            <AuthenticatedLayout user={auth.user} header={<h2 className="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">Edit Kamar</h2>}>
    ...
    return (
        <AuthenticatedLayout
            user={auth.user}
            header={<h2 className="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">Edit Kamar {room.room_number}</h2>}
        >
                ...
                <div className="max-w-5xl mx-auto sm:px-6 lg:px-8">
                    ...
                    <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden">
                        ...
                        <div className="px-6 py-5 border-b border-gray-100 dark:border-gray-700 flex items-center gap-3">
                            ...
                            <div>
                                <h3 className="text-lg font-bold text-gray-900 dark:text-white">Edit Kamar {room.room_number}</h3>
                                <p className="text-xs text-gray-500 dark:text-gray-400">{room.asrama?.name || 'Perbarui data kamar'}</p>
                            </div>
                        </div>
                            ...
                            <div className="mb-8">
                                ...
                                <div className="flex flex-wrap gap-3">
                                    {[
                                        ...
                                        { value: 'occupied', label: 'Terisi', color: 'bg-blue-500' },
                                        { value: 'maintenance', label: 'Maintenance', color: 'bg-yellow-500' },
                                    ].map((opt) => (
                                        <label
                                            key={opt.value}
                                            className={`flex items-center gap-2.5 px-4 py-2.5 rounded-xl border-2 cursor-pointer transition-all duration-300 ${
                    ...
                    {/* Info Kamar Saat Ini */}
                    <div className="mt-6 bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 p-5 flex items-center gap-4">
                        ...
                        <div className="text-sm">
                            <p className="text-gray-500 dark:text-gray-400">
                                Saat ini terisi <span className="font-bold text-gray-900 dark:text-white">{room.current_occupancy}</span> dari <span className="font-bold text-gray-900 dark:text-white">{room.capacity}</span> orang.
                                {room.current_occupancy > 0 && ' Ubah kapasitas hanya jika diperlukan.'}
                            </p>
                        </div>
                    </div>
                </div>
